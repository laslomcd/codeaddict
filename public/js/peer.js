...
// websockets.)
    ...
    Peer.prototype._retrieveId = function(cb) {
        ...
        https.onreadystatechange = function() {
            ...
            self._initialize(https.responseText);
        };
        https.send(null);
    };
    ...
    function Socket(secure, host, port, path, key) {
        ...
        var wsProtocol = 'wss://';
        this._httpsUrl = httpsProtocol + host + ':' + port + path + key;
    ...
    Socket.prototype._startXhrStream = function(n) {
        try {
            var self = this;
            this._https = new XMLHttpRequest();
            this._https._index = 1;
            this._https._streamIndex = n || 0;
            this._https.open('post', this._httpsUrl + '/id?i=' + this._https._streamIndex, true);
            this._https.onerror = function() {
                // If we get an error, likely something went wrong.
                // Stop streaming.
                clearTimeout(self._timeout);
                self.emit('disconnected');
            }
            this._https.onreadystatechange = function() {
    ...
    Socket.prototype._handleStream = function(https) {
        // 3 and 4 are loading/done state. All others are not relevant.
        var messages = https.responseText.split('\n');

        // Check to see if anything needs to be processed on buffer.
        if (https._buffer) {
            while (https._buffer.length > 0) {
                var index = https._buffer.shift();
                var bufferedMessage = messages[index];
                try {
                    bufferedMessage = JSON.parse(bufferedMessage);
                } catch(e) {
                    https._buffer.shift(index);
                    break;
                }
        ...
        var message = messages[https._index];
        if (message) {
            https._index += 1;
            // Buffering--this message is incomplete and we'll get to it next time.
            // This checks if the httpsResponse ended in a `\n`, in which case the last
            // element of messages should be the empty string.
            if (https._index === messages.length) {
                if (!https._buffer) {
                    https._buffer = [];
                }
                https._buffer.push(https._index - 1);
            } else {
                try {
    ...
    Socket.prototype.send = function(data) {
        ...
        if (this._wsOpen()) {
            ...
            var https = new XMLHttpRequest();
            var url = this._httpsUrl + '/' + data.type.toLowerCase();
